...
typedef cl_float cl_unit;

int
invoke (char* device_vender, int device_index, char* cl_file)
{
  printf ("DATA_SIZE: %d\n", DATA_SIZE);
  ...
  cl_init = clock ();
  device_id = retrieve_device(device_vender, device_index, CL_DEVICE_TYPE_GPU);
  ...
  char *kernel_source_int = read_cl_file(cl_file);
...
main (int argc, char** argv)
{
  ...
  if(argc < 4)
	...
	{
	  char* device_vender = argv[1];
	  int device_index = atoi(argv[2]);
	  char* cl_file = argv[3];
	  int error = invoke (device_vender, device_index, cl_file);
	  return error;
	}
}
